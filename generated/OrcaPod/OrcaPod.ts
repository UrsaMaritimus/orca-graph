...
export class OrcaPod extends ethereum.SmartContract {
  static bind(address: Address): OrcaPod {
    return new OrcaPod("OrcaPod", address);
  }
  ...
  ratio(_amount: BigInt): BigInt {
    let result = super.call("ratio", "ratio(uint256):(uint256)", [
  ...
  try_ratio(_amount: BigInt): ethereum.CallResult<BigInt> {
    let result = super.tryCall("ratio", "ratio(uint256):(uint256)", [
      ethereum.Value.fromUnsignedBigInt(_amount)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
